...
**Submitted By**: Shai Livshits

---

## Introduction

Jimi Hendrix (1942-1970) is widely regarded as the most influential electric guitarist in history. In the span of just four years of recording - from *Are You Experienced* (1967) to *Band of Gypsys* (1970) - he redefined what the guitar could sound like, blending blues expressiveness with psychedelic experimentation and raw power. His tone was inseparable from his technique: aggressive picking, whammy-bar manipulation, feedback, and, above all, heavy use of fuzz distortion.

The pedal at the centre of that sound was the **Dallas Arbiter Fuzz Face**, a compact germanium transistor circuit introduced in 1966. Hendrix ran it at the front of his signal chain, feeding a cranked Marshall stack, to produce the dense, sustaining, almost vocal fuzz heard on tracks such as *Purple Haze*, *Foxey Lady*, and *Manic Depression*. The circuit's two germanium transistors clip the audio waveform **asymmetrically** - the positive and negative half-cycles saturate at different levels - which generates a rich mix of both even- and odd-order harmonics. That harmonic content is what gives the Fuzz Face its distinctive warmth and "bloom": notes swell, sustain far beyond their natural decay, and acquire a throaty, singing quality that no later solid-state or digital distortion has fully replicated.

This report documents a digital implementation of the Fuzz Face effect, covering the perceptual qualities of the sound, the underlying DSP signal chain, the mathematical models used, and the design decisions made during implementation.

<div style="page-break-before: always;"></div>

## 1. What the effect does (the sound)

The Fuzz Face is a classic guitar **fuzz** pedal - the aggressive, saturated tone heard on much of Jimi Hendrix's playing. Perceptually it transforms a clean, dynamic input into a thick, buzzy, *sustaining* tone. Two qualities define it:

- **Sustain / compression.** The effect clips the waveform toward a near-square shape, so quiet parts are lifted and loud parts are limited. Notes appear to "bloom" and ring on far longer than the dry signal. On the supplied *Purple Haze* guitar sample, the crest factor (peak-to-RMS ratio) drops from 15.1 dB dry to 8.4 dB wet - the dynamics are strongly compressed, which the ear hears as sustain and "thickness".
- **Harmonic richness with a vocal character.** Because the clipping is **asymmetric**, the output contains both **even- and odd-order harmonics** (Figure 3). The even harmonics give the Fuzz Face its slightly hollow, vocal colour that distinguishes it from purely symmetric distortions. It works best on single notes; on full chords the same nonlinearity also produces inharmonic intermodulation tones, which is why fuzz is most often used for lead lines and power chords.
...
The transfer curve is shown in Figure 1 alongside other models for comparison. The `tube` model (DAFX Eq. 4.13) uses an exponential curve with a bias point, giving a milder, preamp-like asymmetry - the positive half-wave stays nearly linear while the negative is compressed, resulting in less fuzz and more of a valve overdrive character. The `exp` and `softclip` models are both symmetric, so they produce only odd harmonics and lack the vocal warmth of the Fuzz Face; `exp` uses a smooth exponential saturation while `softclip` transitions through three distinct regions (linear, polynomial, and hard-limit). The `hard` model is a simple brickwall clip to $\pm 1$, the most aggressive of the five - it generates the strongest odd harmonics but sounds harsh and digital compared to the smooth soft-clippers.
...
<code>oversample=1</code> (dark) produces dense inharmonic aliasing; <code>oversample=8</code> (orange) is clean.</em>
</p>
...
- **Clipping models:** five models are implemented and selectable via `--model`:
  - `asym` (default) - asymmetric $\tanh$ clip ($g = 20$, $\alpha = 0.6$); produces even + odd harmonics and strong sustain, modelling the Fuzz Face directly.
  - `tube` - DAFX Eq. 4.13 asymmetric exponential curve with bias point $Q$ and hardness `dist`; milder, preamp-like asymmetry that leaves the positive half-wave nearly linear.
  - `exp` - DAFX Eq. 4.15 symmetric exponential soft clip $\text{sgn}(x)(1-e^{-|x|})$; smooth saturation, only odd harmonics.
  - `softclip` - Schetzen three-region symmetric overdrive (linear / polynomial / hard-limit); gentler onset than the others.
  - `hard` - hard clip to $[-1, 1]$; maximally aggressive, brickwall saturation.
...
- **Oversampling factor:** $N = 8$ by default - a good trade-off between alias
  suppression and CPU cost; selectable via `--oversample`.
...
While the five clipping models differ significantly in their mathematical formulation - from a simple hard clip to the bias-point exponential of the tube model - the perceptual difference between them is noticeable but not dramatic. In practice, all models produce a recognisable fuzz character, and choosing the "best" sounding one turned out to be genuinely difficult. The `asym` model was selected as the default because its tanh-based asymmetric saturation gives the clearest even-harmonic content and the most sustain, but a listener switching between models would likely find all of them musically usable.
